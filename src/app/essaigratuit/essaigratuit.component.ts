...
@Component({
  selector: 'app-essaigratuit',
  templateUrl: './essaigratuit.component.html',
  styleUrls: ['./essaigratuit.component.css']
})
export class EssaigratuitComponent implements OnInit {
  boutique1: Boutique;
    ...
    this.activatedRoute.params.subscribe(
      (params) => {
        ...
        this.aboService.addAbo1(params.id).subscribe(
          (response) => {
            this.status1 = true;
            const link = ['boutique' + `/${this.boutique1.id}` + '/' + 'essaigratuit' + '/' + 'valideressai'];
            this.router.navigate(link);
          },
          (error) => {
          }
